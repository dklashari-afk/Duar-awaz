...
Single-file Flask app (Vercel-ready) — mirrors the original project's
deploy structure (app.py at root, api/index.py imports `app`, same
vercel.json / requirements.txt) so the existing deploy guide still works.
"""
...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "daur-awaz-vercel-2024-change-me")

# Vercel's filesystem is read-only except /tmp, and /tmp is ephemeral
# (wiped on cold start) — same limitation as the original project.
# Photos are therefore stored as base64 inside the same SQLite row
# rather than as separate files, so they survive exactly as long as
# the rest of the data does. For permanent storage, swap this for
# Vercel Postgres + a blob store (S3 / Cloudinary) later.
#
# On Vercel (VERCEL env var is set automatically) we use /tmp, which
# exists there. On Windows/Mac/Linux running locally, /tmp either
# doesn't exist or isn't the right place — so locally we use a plain
# file next to app.py instead. This keeps one codebase working both
# on Vercel and on your own PC without any manual edits.
if os.environ.get("VERCEL"):
    DB_PATH = "/tmp/daur_awaz_final.db"
else:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DB_PATH = os.path.join(BASE_DIR, "daur_awaz_final.db")

app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + DB_PATH.replace("\\", "/")
db = SQLAlchemy(app)

# ------------------------------------------------------------------
# CONFIG — change these for your own council
# ------------------------------------------------------------------
CITY_NAME_UR = "دوڑ"
CITY_NAME_EN = "Daur"
...
CATEGORIES = [
    ...
    ("Bijli / Street Light", "fa-lightbulb"),
    ("Road / Gali", "fa-road"),
    ("Sewerage", "fa-water"),
...
# Fallback seed login — only used to create the FIRST admin account the
# very first time the app runs (empty database). After that, all admin
# accounts (including this one) live in the AdminUser table and are
# managed from the dashboard — see /admin/users and /admin/account.
...
class Complaint(db.Model):
    ...
    photo_data = db.Column(db.Text)          # base64 data URI, optional
    status = db.Column(db.String(20), default="Pending")
    remarks = db.Column(db.Text)             # official chairman reply
...
class AdminUser(db.Model):
    ...
    role = db.Column(db.String(20), default="Assistant")   # "Chairman" or "Assistant"
    created_at = db.Column(db.DateTime, default=datetime.utcnow)


with app.app_context():
    db.create_all()
    # First-run seed: if there are no admin accounts yet, create one
    # from the ADMIN_USERNAME / ADMIN_PASSWORD env vars (or the
    # admin/admin123 default). This only ever runs once — after the
    # first account exists, this block does nothing.
...
<header class="bg-white border-b sticky top-0 z-50 shadow-sm">
  ...
  <div class="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex justify-between items-center gap-3">
    ...
    <nav class="flex items-center gap-2 sm:gap-4">
      {link('/', 'Home', 'ہوم', 'home')}
      {link('/track', 'Track Complaint', 'ٹریک کریں', 'track')}
...
<section class="bg-gradient-to-b from-white to-[#F6F8F7] border-b">
  <div class="max-w-6xl mx-auto px-6 py-10 text-center">
    ...
    <h2 class="urdu text-2xl sm:text-3xl text-gov-dark font-bold mb-2">آپ کی آواز، براہ راست چیئرمین تک</h2>
    <p class="text-gray-600 max-w-2xl mx-auto text-sm sm:text-base">Report civic problems in Daur city — sanitation, water, electricity, roads — and track their resolution in real time, just like the Pakistan Citizen's Portal.</p>
  </div>
</section>
...
def submit():
    ...
    if file and file.filename:
        raw = file.read()
        # keep the DB light — cap at ~2MB per photo
...
# ------------------------------------------------------------------
# TRACK COMPLAINT (standalone lookup + timeline)
# ------------------------------------------------------------------
TRACK_HTML = """
...
# ------------------------------------------------------------------
# MY ACCOUNT — change your own username / password
# ------------------------------------------------------------------
ACCOUNT_HTML = """
...
# ------------------------------------------------------------------
# MANAGE USERS — create an account for a PA / staff member
# ------------------------------------------------------------------
USERS_HTML = """
...
def users_delete(id):
    ...
    if id == session.get("admin_id"):
        return redirect("/admin/users")  # can't delete yourself from here
...
# ------------------------------------------------------------------
# REPORTS — monthly & yearly progress (Pending / In-Progress / Resolved)
# ------------------------------------------------------------------
import calendar
